...
vector<double> log_likelihood(PFSA& G, const symbol_list_& s)
{
	if(s.empty())
	{
	...
	vector<double> state_llks(numstates, 0);

    for(unsigned int init_st = 0; init_st < numstates; init_st++)
	{
		vector<double> curr_state(numstates, 0);
		curr_state[init_st] = 1.;
		cout << curr_state[init_st] << endl;
		getchar();
...
int main(int argc, char *argv[])
{
	...
	{
    	llk = log_likelihood(G, Svec[i].get_symbol_list());
		cout << llk << endl;
	}
