...
# Run-length PFSA code followed by an iid deletion channel.
# The encoder and stochastic regime decoder are otherwise unchanged.
#
...
DELETE_Q="${DELETE_Q:-0.01}"
DELETE_SEED="${DELETE_SEED:-1}"
...
# Downstream iid deletion channel. The decoder receives only the thinned
# stream and is not given deletion locations. Keep the pre-channel stream
# solely for evaluation.
mv "$OUT/transmission.dat" "$OUT/transmission_predelete.dat"
python3 - "$OUT" "$DELETE_Q" "$DELETE_SEED" <<'PY'
from pathlib import Path
import random,sys
out=Path(sys.argv[1]); q=float(sys.argv[2]); seed=int(sys.argv[3])
if not (0.0 <= q < 1.0): raise SystemExit("DELETE_Q must satisfy 0 <= q < 1")
rng=random.Random(seed)
x=(out/"transmission_predelete.dat").read_text().split()
y=[s for s in x if rng.random() >= q]
(out/"transmission.dat").write_text(" ".join(y)+"\n")
(out/"deletion_channel.tsv").write_text(
    "q\tseed\tinput_symbols\toutput_symbols\trealized_deletion_fraction\n"
    f"{q}\t{seed}\t{len(x)}\t{len(y)}\t{1-len(y)/max(1,len(x)):.10f}\n")
PY
...
python3 - "$OUT" "$BASE_LEN" "$SCAN_STEP" "$MIN_RUN_WINDOWS" "$DELETE_Q" <<'PY'
from pathlib import Path
import sys, statistics
out=Path(sys.argv[1]); L=int(sys.argv[2]); step=int(sys.argv[3]); minw=int(sys.argv[4]); q=float(sys.argv[5])
Leff=(1.0-q)*L
meta=[]
...
echo "  deletion_channel.tsv   downstream deletion statistics"
echo "  transmission_predelete.dat  encoder output before deletion, evaluation only"
