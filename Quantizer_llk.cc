...
const string VERSION="\nQuantizer with log-likelihood v1.0 ";
...
void normalize_inplace(vector<double> & vec)
{
  ...
  // double var = (sq_sum - mean * mean) / (vec.size() - 1);
  // double std = sqrt(var);
  // for (size_t s = 0; s < vec.size(); s++)
  //   {
  //     vec[s] = (vec[s] - mean) / std;
  //   }
}
...
pair<vector<vector<double>>, vector<int>> get_data(string data_directory, unsigned int dataType) 
{
  ...
  if (dataType == 0)
    {	
      ifstream input(data_directory + "/dataset");	
      string line;
  ...
  else
    {
      ...
      ifstream libList(data_directory + "/library_list");
      string line;
      ...
      while (getline(libList, line))
	{
	  ...
	  string libFilename;
	  ss >> libFilename;
	  filenames.push_back(data_directory + "/" + libFilename);
	  int label;
	  ss >> label;
	  unsigned int count;
	  ss >> count;
	  if (dataType == 1)
		...
		{
		  labels.push_back(overallCount + i);
		}
	      overallCount += count;
	    }
	  else
  ...
  return pair<vector<vector<double>>, vector<int>>(data, labels);
}

template <class T>
pair<vector<T>, vector<int>> down_sampling(
					   const vector<T> & data,
					   const vector<int> & labels,
					   double percentage)
{
  unsigned int count = data.size();
  unsigned int k = (int)(count * percentage);
  k = (k > 1)? k : 1;
  vector<unsigned int> chosen = choose(count, k);
	
  vector<T> sampled_data;
  vector<int> sampled_labels;
  for (size_t c : chosen)
    {
      sampled_data.push_back(data[c]);
      sampled_labels.push_back(labels[c]);
    }
  return pair<vector<T>, vector<int>>(sampled_data, sampled_labels);		
}

template <class T>
pair<vector<T>, vector<int>> down_sampling_labeled(
						   const vector<T> & data,
						   const vector<int> & labels,
						   double percentage)
{
  map<int, vector<unsigned int>> index_map;
  map<int, vector<unsigned int>>::iterator itr;
  for (size_t i = 0; i < labels.size(); i++)
    {
      unsigned int label = labels[i];
      itr = index_map.find(label);
      if (itr == index_map.end())
	{
	  index_map[label] = vector<unsigned int>(1, i);
	}
      else
	{
	  index_map[label].push_back(i);
	}
    }
  map<int, vector<unsigned int>> down_sample_indices;
  for (itr = index_map.begin(); itr != index_map.end(); ++itr)
    {
      unsigned int label = itr->first;
      vector<unsigned int> indices = itr->second;
      unsigned int count = indices.size();
      unsigned int k = (int)(count * percentage);
      k = (k > 1)? k : 1;
      vector<unsigned int> chosen = choose(count, k);
      vector<unsigned int> chosen_indices;
      for (size_t c : chosen)
	{
	  chosen_indices.push_back(indices[c]);
	}
      down_sample_indices[label] = chosen_indices;
    }
	
  vector<T> sampled_data;
  vector<int> sampled_labels;
  for (itr = down_sample_indices.begin(); itr != down_sample_indices.end(); ++itr)
    {
      for (size_t i : itr->second)
	{
	  sampled_data.push_back(data[i]);
	  sampled_labels.push_back(labels[i]);
	}
    }
  return pair<vector<T>, vector<int>>(sampled_data, sampled_labels);		
}
...
const unsigned int REPEAT = 20;
const double EPSILON = 1e-2;

typedef struct {double sae; double discrimination; vector<vector<double>> dist_matrix;} stat;
class Quantizer
{
protected:
  	// The raw data for quatization.
  	vector<vector<double>> data;

  	// For supervised learning, the label will be read from the library_list file.
  	// For unsupervised learning the label is just an ordering of the data streams.
  	vector<int> labels;
	...
	// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Out-dated ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
	// The depth for calculating norm.
  	unsigned int depth;
  	// The number of repetitions for calculating norm.
  	unsigned int repeat;
	// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Out-dated ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
	

	// Calculate the distance matrix, sae, and discrimination of the quantization.
  	void _calc_statistics(double percentage, unsigned int dataType);
  	pair<bool, stat> _calc_statistics(const vector<double> & partition, double percentage, unsigned int dataType);

public:
  	map<unsigned int, pair<bool, vector<double>>> partitions;
  	map<unsigned int, pair<bool, stat>> Stats;
  	// Constructor:
  	Quantizer(
		vector<vector<double>> data,
		vector<int> labels,
...
void Quantizer::_calc_statistics(double percentage, unsigned int dataType)
{
  	for (size_t i = 0; i < partitions.size(); i++)
  	{
      	if (partitions[i].first)
		{
	  		Stats[i] = _calc_statistics(partitions[i].second, percentage, dataType);
		}
      	else
		{
	  		stat empty;
	  		Stats[i] = pair<bool, stat>(false, empty);
		}
    }
}


pair<bool, stat> Quantizer::_calc_statistics(const vector<double> & partition, double percentage, unsigned int dataType)
{
	pair<bool, vector<symbol_list_>> quantized_pair = _fit(partition);
  	bool is_successful = quantized_pair.first;
		
  	stat S;
  	if (is_successful)
    {
      	vector<symbol_list_> quantized_data = quantized_pair.second;
		pair<vector<symbol_list_>, vector<int>> sampled_pair;
      	if (dataType == 0 || dataType == 1)
		{
	  		sampled_pair = down_sampling(quantized_data, labels, percentage);
		}
      	else
		{
	  		sampled_pair = down_sampling_labeled(quantized_data, labels, percentage);
		}
      	vector<symbol_list_> sampled_data = sampled_pair.first;
      	vector<int> sampled_labels = sampled_pair.second;								

      	vector<vector<double>> dist_matrix = get_llk_distance_matrix(PFSA_vec, sampled_data) ;
		...
		{
	  		for(unsigned int j = i; j < sampled_data.size(); j++)
	    	{
	      		if(sampled_labels[i] == sampled_labels[j])
				{
		  			sae += (dist_matrix[i][j] + dist_matrix[j][i]) / 2.0;
		  			s_count++;
				}
	      		else
				{
		  			discrimination += (dist_matrix[i][j] + dist_matrix[j][i]) / 2.0;
		  			d_count++;
				}
	    	}
		}
      	sae /= s_count;
      	discrimination /= d_count;
		
      	S.sae = sae;
      	S.discrimination = discrimination;
      	S.dist_matrix = dist_matrix;
   	}	
  	return pair<bool, stat>(is_successful, S);
}
	...
	bool verbose)
{
	...
	this->PFSA_vec = read_PFSA(PFSA_directory);
  	...
  	_calc_statistics(percentage, dataType);
}
...
{ 
  try
    ...
    ("version,V", "print version number");
      ...
      usg.add_options()
    ("type,t", value<unsigned int>(&dataType), "0 for unsupervised dataset, 1 for supervised problem treated as unsupervised, 2 for supervised.")
    ("data-directory,D", value<string>(&data_directory), "directory of the dataset")
    ("pfsa-directory,P", value<string>(&PFSA_directory), "directory of the model PFSA")
    ("percentage,x", value<double>(&percentage)->default_value(1), "A float value between [0, 1].")
    ...
    ("output,o", value<string>(&output)->implicit_value("dist"), 
     "-o to output distance matrices with names [parameter]_dist, -o[Name] to output distance matrices with names [parameter]_[Name], missing for no output.")
    ("best_num,M", value<unsigned int>(&best_num), "the number of best parameters to output");


      options_description desc( "\n### Quantizer ### (yi huang 2018)");
      ...
      else
    {
      ...
      // Read in data according to the specified data type
      if(data_directory.back() != '/')
    {
      data_directory += "/";
    }
    	
      pair<vector<vector<double>>, vector<int>> data_label_pair = get_data(data_directory, dataType);
      vector<vector<double>> data = data_label_pair.first;
      vector<int> labels = data_label_pair.second;

      // Produce iquantiles for all alphabet size between ASLow and ASHigh 
      ...
      if ((_pruned.size() == 2) || ((_pruned.size() == 1) & _pruned[0]))
    {
      ...
      map<unsigned int, double> ratio_map;
      ...
      for (bool r : _pruned)
    {
      for (size_t d : _detrended)
          ...
          for (bool n : _normalized)
    	{
    	  ...
    	  Quantizer q(processed_data, labels, PFSA_directory, epsilon, quantiles, percentage, dataType, verbose);
    				
    	  for (size_t i = 0; i < q.Stats.size(); i++)
    	    {
    	      if (q.Stats[i].first)
    		{
    		  ...
    		  double sae = q.Stats[i].second.sae;
    		  double discr = q.Stats[i].second.discrimination;		
    		  if (verbose)
    		    {	
    		      cout << count << "\t" <<  parameter 
    			   << "\tsae = " << sae 
    			   << "\tdiscr = " << discr 
    			   << "\tratio = " << sae / discr << endl;
    		    }
    		  parameter_map[count] = parameter;
    		  ratio_map[count] = sae / discr;
    		  dist_map[count] = q.Stats[i].second.dist_matrix;
    		  count++;
    		}
    	      else
    		{
      ...
      // Ordering the partitions according the increasing order of the sae-to-discr ratio;	
      vector<unsigned int> idx;
      for (map<unsigned int, double>::iterator itr = ratio_map.begin(); 
       itr != ratio_map.end(); ++itr)
    {
      idx.push_back(itr->first);
    }
      sort(idx.begin(), idx.end(), 
       [&ratio_map](unsigned int a, unsigned int b) 
       {
         return ratio_map[a] < ratio_map[b];
       });
    	
      if (vm.count("best_num"))
    {
      best_num = (idx.size() < best_num)? idx.size() : best_num;
    }
      else
    {
      best_num = idx.size();
      ...
      if (verbose)
    {
      cout << "find " << idx.size() << " valid partitions and output " << best_num << " valid partitions" << endl;
    }

      ofstream parameter_file(data_directory + "/valid_parameter");
      for (size_t i = 0; i < best_num; i++)
    {
      string parameter = parameter_map[idx[i]];
      parameter_file << parameter << endl;
      if (output.size() != 0)
        {
          ofstream dist_file(data_directory + "/" + parameter + "_" + output);
          vector<vector<double>> dist = dist_map[idx[i]];
          for (size_t i = 0; i < dist.size(); i++)
    	{
    	  for (size_t j = 0; j < dist[i].size(); j++)
    	    {
    	      dist_file << dist[i][j];
    	      (j < dist[i].size() - 1) ? dist_file << " " : dist_file << "\n";					
    	    }
