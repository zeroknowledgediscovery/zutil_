...
const string VERSION="\nQuantizer with log-likelihood v1.1 \n\t Initially designed to work with EEG/data/Motor_Movement_Imagery";
...
pair<vector<vector<double>>, vector<int>> get_data(string data_directory) 
{
	// The data_directory must contain files with names 
	// 	1. library_list;
	// 	2. test_data;
	
	// The library_list file details the information of training data and must have the format
	// 	[filename_1] [class_1] [size_1]
	//	[filename_2] [class_2] [size_2]
	//	etc.
	// where [filename_i]s are the names for the training data files (row-wise),
	// [class_i]s are the class labels for the training data, and
	// [size_i]s are the numbers of rows (samples) in each of training data files.
	// The class labels in the middle column of library_list can be any integers.

	// The file test contains test data (row-wise).
	
	string line;

	// Read the library_list file:
	ifstream libList(data_directory + "/library_list");
	vector<string> libFilenames;
    ...
    while (getline(libList, line))
	{
		...
		{
			train_labels.push_back(train_label);
		}
	}
	libList.close();		
	...
	{
		ifstream train_data_file(data_directory + "/" + libFilenames[i]);	
	...
	ifstream test_data_file(data_directory + "/test_data");	
	...
	// ifstream test_label_file(data_directory + "/test_labels");	
	// 
	// getline(test_label_file, line);
	// stringstream ss;
	// ss << line;
	// int test_label;
	// 
	// while (ss >> test_label)
	// {
	// 	test_labels.push_back(test_label);
	// }

	// test_label_file.close();

	return make_pair(data, train_labels);
}
...
protected:
	...
	// Calculate the distance matrix, sae, and discrimination of the quantization.
  	...
  	pair<bool, stat> _calc_statistics(const vector<double> & partition);
...
void Quantizer::_calc_statistics()
{
  	for (size_t i = 0; i < partitions.size(); i++)
  	{
		...
		{
	  		Stats[i] = _calc_statistics(partitions[i].second);
		}
      	else
		{
	  		stat empty;
...
pair<bool, stat> Quantizer::_calc_statistics(const vector<double> & partition)
{
  	...
  	if (is_successful)
    {
		...
		{
	  		for(unsigned int j = i; j < labels.size(); j++)
	    	{
				...
				{
		  			discrimination += dist_matrix[i][j];
		  			d_count++;
				}
	    	}
		}
...
{ 
  try
      ...
      if(data_directory.back() != '/')
    {
      ...
      pair<vector<vector<double>>, vector<int>> data_pair = get_data(data_directory);
      vector<vector<double>> data = data_pair.first;
      vector<int> train_labels = data_pair.second;
      ...
      for (bool r : _pruned)
    {
      for (size_t d : _detrended)
          ...
          for (bool n : _normalized)
    	{
    	  ...
    	  Quantizer q(processed_data, train_labels, PFSA_directory, epsilon, quantiles, verbose);
